...
let printUsage (msg:string option) (argInfo:ArgInfo list) =
    stringB {
        ...
        for aI in first do
            if not aI.Hidden then yield! printArgUsage aI
...
let printAppSettings (argInfo : ArgInfo list) printComments (args : 'Template list) =
    let printEntry (t : 'Template) : XNode list =
        ...
        | Some key ->
            let values =
                if fields.Length = 0 then "true" else
            ...
            if not printComments then [xelem] else
            let comment =
                stringB {
                    yield ' '; yield aI.Usage
                    match aI.FieldParsers |> Array.toList with
                    | [] -> ()
                    | first :: rest ->
                        yield " : "; yield string first
                        for p in rest do
                            yield ", "; yield string p
                    yield ' '
                } |> String.build
